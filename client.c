...
int main(int argc, char *argv[]) {
    ...
    int retval = client_get_config_value("./config", "master", host_ip);
    _PRINT("host_ip is %s\n", host_ip);
    ...
    while (1) {
         ...
         * */
        int recv_lens = client_recv_msg(client_control_socket, recvline, MAXLINE, 0);
        _PRINT("recvline : %s\n", recvline);
        ...
        char ack_buf[64];
        int ack_stat = client_find_str(recvline, "=", ack_buf);
        printf("ack_buf :%s\n", ack_buf);
        fflush(stdout);
